    ...
    <div className="flex border-b border-gray-100 last:border-b-0" style={{ gap: '1.2vw', padding: '1.2vw 0' }}>
      ...
      <div className="flex-1 flex flex-col justify-center">
        <Link href={`#`} className="no-underline">
          <h4 className="font-semibold text-gray-900 leading-snug hover:text-[#00A4E4] transition-colors" style={{ fontSize: '1vw', marginBottom: '0.4vw' }}>
            {conversation.title}
          </h4>
        </Link>
...
export function ConversationList({ title, conversations, showViewAll = false }) {
  return (
    <div className="bg-white" style={{ borderRadius: '1.2vw', padding: '1.8vw', boxShadow: '0 0.2vw 1vw rgba(0,0,0,0.03)' }}>
      <div className="flex justify-between items-center" style={{ marginBottom: '1vw' }}>
        <h3 className="font-semibold text-gray-800 uppercase" style={{ fontSize: '0.85vw', letterSpacing: '0.05em' }}>
          {title}
        </h3>
        {showViewAll && (
          <button className="flex items-center text-gray-500 bg-white border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors" style={{ gap: '0.4vw', fontSize: '0.7vw', padding: '0.4vw 1vw', borderRadius: '1.5vw' }}>
            VIEW ALL
